...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'rajukumar56085@gmail.com' ")
...
def TakeImages():
    ...
    columns = ['SERIAL NO.', '', 'register no.', '', 'NAME']
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        res = "Images Taken for R0LL NO: " + Id
        row = [serial, '',register, '', name]
...
def TrackImages():
    ...
    col_names = ['REGISTER NO.', '', 'Name', '', 'Date', '', 'Time']
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                ID = df.loc[df['SERIAL NO.'] == serial]['REGISTER NO.'].values
                ID = str(ID)
                ID = ID[1:-1]
                bb = str(aa)
                bb = bb[2:-2]
